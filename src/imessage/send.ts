...
export type IMessageReactionOpts = Pick<
  IMessageSendOpts,
  "cliPath" | "dbPath" | "accountId" | "timeoutMs" | "client" | "config" | "account" | "createClient"
>;

export async function sendIMessageReaction(
  to: string,
  messageId: string,
  reaction: string,
  opts: IMessageReactionOpts = {},
): Promise<{ ok: boolean }> {
  const cfg = opts.config ?? loadConfig();
  const account =
    opts.account ??
  ...
  const shouldClose = !opts.client;
  try {
    const result = await client.request<{ ok?: string }>("react", params, {
      timeoutMs: opts.timeoutMs,
    });
    return {
