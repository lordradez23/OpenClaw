...
async function restartLoginSocket(login: ActiveLogin, runtime: RuntimeEnv) {
  const maxAttempts = 5;
  if (login.restartAttempts >= maxAttempts) {
    return false;
  }
  login.restartAttempts++;
  // Quadratic backoff with some jitter.
  const delayMs = 1000 * Math.pow(login.restartAttempts, 1.5) + Math.random() * 1000;
  runtime.log(
    info(
      `WhatsApp asked for a restart after pairing (code 515); retry attempt ${login.restartAttempts}/${maxAttempts} in ${Math.round(delayMs)}ms…`,
    ),
  );
  closeSocket(login.sock);
