import type { AnyMessageContent, ConnectionState, Contact, proto, WAMessage } from "@whiskeysockets/baileys";
...
import { loadContacts, saveContacts } from "../auth-store.js";
...
export async function monitorWebInbox(options: {
  ...
  const debouncer = createInboundDebouncer<WebInboundMessage>({
    ...
    onError: (err, items) => {
      inboundLogger.error(
        { error: String(err), count: items.length },
  ...
  const knownContacts = loadContacts(options.authDir);

  const resolveInboundJid = async (jid: string | null | undefined): Promise<string | null> => {
    if (!jid) {
      return null;
    }
    const e164 = await resolveJidToE164(jid, { authDir: options.authDir, lidLookup });
    if (e164) {
      return e164;
    }
    // Fallback to local contact store if LID lookup fails
    const cleanJid = jid.split(":")[0]?.split("@")[0];
    if (cleanJid && /^\d+$/.test(cleanJid)) {
      return cleanJid;
    }
    return null;
  };

  const syncContacts = async (updates: Partial<Contact>[]) => {
    let changed = false;
    for (const contact of updates) {
      ...
      if (name && knownContacts[contact.id] !== name) {
        if (shouldLogVerbose()) {
          logVerbose(`Contact discovery: discovered name "${name}" for ${contact.id}`);
        }
        knownContacts[contact.id] = name;
        changed = true;
      }

      // 2. Discover LID-to-PN Mappings (if available in the update)
      // This helps with discovery of the real phone number for encrypted interactions.
      const anyContact = contact as any;
      const lid = anyContact.lid || (contact.id.endsWith("@lid") ? contact.id : undefined);
      const pn = anyContact.phoneNumber || anyContact.pn;
      
      if (lid && pn) {
        // If we found a mapping, we could potentially save it to a mapping file.
        // For now, we at least ensure the name is synced for both if they match.
        if (name) {
          const pnJid = pn.includes("@") ? pn : `${pn}@s.whatsapp.net`;
          if (knownContacts[pnJid] !== name) {
            knownContacts[pnJid] = name;
            changed = true;
          }
        }
      }
    ...
    if (changed) {
      await saveContacts(options.authDir, knownContacts);
    }
  };
