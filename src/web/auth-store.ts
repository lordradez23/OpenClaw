...
export async function saveContacts(authDir: string, contacts: Record<string, string>) {
  const filePath = path.join(resolveUserPath(authDir), "contacts.json");
  try {
...
export function loadContacts(authDir: string): Record<string, string> {
  const filePath = path.join(resolveUserPath(authDir), "contacts.json");
  try {
    if (!fsSync.existsSync(filePath)) {
      return {};
